...
- `release-branch.yml` (agregado 2026-07-15): cuando un PR se **mergea** hacia una rama `release/*` (fixes de estabilización, no el corte inicial), se taguea como `v<version>-rc`, se crea un GitHub Release marcado `prerelease`, y se despliega a **Staging** (no a producción — ver `docs/Deployment.md`, ambiente `Staging` = "validación previa a producción"). Requiere secrets `FIREBASE_SERVICE_ACCOUNT_STAGING`/`FIREBASE_PROJECT_ID_STAGING`; se omite sin fallar mientras no existan. No repite tests: ya corrieron como parte del PR (`pull_request:` de `backend.yml`/`web.yml`/`android.yml` no filtra por rama destino).
