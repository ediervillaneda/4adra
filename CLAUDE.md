...
**4adra** (Cuentas claras, gastos compartidos) es, por ahora, un repositorio de especificación: solo contiene `docs/`, sin código fuente, `package.json`, build files ni tests todavía. No existen comandos de build/lint/test que ejecutar — la tarea actual es diseño y documentación, no implementación. Cuando se agregue código (backend `backend/src/`, cliente Android, cliente Web), este archivo debe actualizarse con los comandos reales (gestor de paquetes, lockfile, scripts de test por capa, emuladores de Firebase, etc.); no inventar comandos que no existan en el repo.
